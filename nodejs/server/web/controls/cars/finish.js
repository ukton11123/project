...
exports.get=function (client,userCode,passWD) {
    var userMoney,winID,poolMoney,petMoney
     var winMoney=0
    var SQL = "select state,winID,poolMoney from Gamecars";
    mysql.query(SQL, function (er, row) {
        ...
        //获取下注总数
        SQL="select SUM(Money) petMoney from GamecarsPet where userCode="+userCode;
        mysql.query(SQL, function (err, row) {
            if (err)
                return client.error("操作失败。");
            petMoney=row.petMoney?row.petMoney:0;
            //获取获胜金币
            SQL="select SUM(Money) petMoney,ID from GamecarsPet where userCode="+userCode+" and ID="+winID+"  group by ID";
            mysql.query(SQL, function (er, row) {
                if (row)
                {
                    var rate=(row.ID+1)*2;
