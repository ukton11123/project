...
exports.get=function (client,userCode,passWD,petID,petMoney) {
    ...
    var SQL = "select state from GameHorse";
        ...
        common.getUserMoney(userCode, passWD, function (err, money) {
            ...
            common.setUserMoney(userCode, userMoney, function (err) {
                if (err)
                    return client.error("操作失败。");
                //保存交易记录
                common.saveMoneyRecord(10, "马场下注", userCode, -petMoney, userMoney);
                //创建下注
                var Data = {userCode: userCode, ID: petID, Money: petMoney,Rate:global.horseRateArr[petID]}
                mysql.insert("GameHorsePet", Data, function (err) {
                    if (err)
                        return client.error("操作失败。");
